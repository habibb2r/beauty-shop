  ...
  const handleDeleteProduct = (item) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axiosSecure.delete(`/deleteItem/${item._id}`).then((res) => {
              ...
              <tr key={item._id}>
                ...
                <td>
                  <span className="badge badge-ghost badge-sm">
                    {item?.price.onOffer ? "On Offer" : "Not On Offer"}
                  </span>
                  <br />
                  <span className="badge badge-ghost badge-sm">
                ...
                <td>
                  <div className="flex flex-col justify-start items-start gap-3">
                    <span className="font-bold">
                      {item?.price.present_price}$
                    </span>
                    <span className="line-through font-semibold">
                      {item?.price.previous_price}$
                    </span>
                  </div>
                </td>
                <th>
                  <div className="flex justify-start items-center gap-5">
                    <Link
                      to={`/sellerdashboard/updateitem/${item._id}`}
                      state={{ item }}
                    >
                      <img
                        className="h-[40px]"
                    ...
                    <button onClick={() => handleDeleteProduct(item)}>
                      <img className="h-[40px]" src={deleteImg} alt="" />
                    </button>
                  </div>
                </th>
              </tr>
